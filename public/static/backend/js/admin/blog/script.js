var index = {
    ...
    remove: function (id) {
        ...
        swal.fire({
            ...
            html: 'Khi tin tức bị xóa, bạn sẽ không thể khôi phục lại được.' +
                "<br>" +
            ...
            if (result.value) {
                if (result.value) {
                    $.ajax({
                        url: laroute.route('admin.blog.destroy'),
                        method: 'POST',
                        dataType: 'JSON',
                        data: {
                            id: id
    ...
    submit_load: function () {
        $.getJSON('/admin/validation', function (json) {
            ...
            form.validate({
                ...
                messages: {
                    category_id: {
                        required: json.blog.category_required,
                    },
                    title_vi: {
            ...
            $.ajax({
                url: laroute.route('admin.blog.store'),
                method: 'POST',
                dataType: 'JSON',
                data: {
                ...
                success: function (res) {
                    if (res.error == false) {
                        swal.fire(json.blog.success, "", "success").then(function (result) {
                            if (result.dismiss == 'esc' || result.dismiss == 'backdrop') {
                                window.location.reload();
                            }
                            if (result.value == true) {
                        ...
                        swal.fire(json.blog.fail, '', "error");
                    }
                },
                error: function (res) {
                    var mess_error = '';
                    ...
                    swal.fire(json.blog.fail, mess_error, "error");
                }
            });
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.error == false) {
                        swal.fire(json.blog.success, "", "success").then(function (result) {
                            if (result.dismiss == 'esc' || result.dismiss == 'backdrop') {
                                window.location.href = laroute.route('admin.blog');
                            }
                            if (result.value == true) {
                                window.location.href = laroute.route('admin.blog');
                            }
...
var update = {
    ...
    submit: function (id) {
        $.getJSON('/admin/validation', function (json) {
            ...
            $.ajax({
                url: laroute.route('admin.blog.update'),
                method: 'POST',
                dataType: 'JSON',
                data: {
                    title_vi: $('#title_vi').val(),
                ...
                success: function (res) {
                    if (res.error == false) {
                        swal.fire(json.blog.update_success, "", "success").then(function (result) {
                            if (result.dismiss == 'esc' || result.dismiss == 'backdrop') {
                                window.location.href = laroute.route('admin.blog');
                            }
                            if (result.value == true) {
                        ...
                        swal.fire(json.blog.update_fail, '', "error");
                    }
                },
                error: function (res) {
                    var mess_error = '';
                    ...
                    swal.fire(json.blog.update_fail, mess_error, "error");
                }
            });
...
function uploadAvatar(input) {
    if (input.files && input.files[0]) {
        ...
        if (Math.round(fsize / 1024) <= 10240) {
            $('.error_img').text('');
            $.ajax({
                url: laroute.route("admin.blog.upload"),
                method: "POST",
                data: form_data,
                contentType: false,
                cache: false,
